#!/bin/bash
# ==============================================================================
# NOVA VPN — Установка легковесного Live WGDashboard (порт 8088)
# ==============================================================================

set -e

echo "🚀 Установка легковесного WGDashboard для AmneziaWG..."

mkdir -p /opt/nova-dashboard

cat <<'PY_EOF' > /opt/nova-dashboard/app.py
import os
import sys
import json
import time
...
# Создаем systemd юнит для автозапуска
cat <<'SERVICE_EOF' > /etc/systemd/system/nova-dashboard.service
[Unit]
Description=NOVA VPN Live WGDashboard Service
After=network.target docker.service

[Service]
Type=simple
User=root
WorkingDirectory=/opt/nova-dashboard
ExecStart=/usr/bin/python3 /opt/nova-dashboard/app.py
Restart=always
RestartSec=5
Environment=DASHBOARD_PORT=8088
Environment=DASHBOARD_USER=admin
Environment=DASHBOARD_PASS=nova2026
...
if command -v ufw >/dev/null 2>&1; then
    ufw allow 8088/tcp comment 'NOVA WGDashboard' >/dev/null 2>&1 || true
fi

# Перезапускаем systemd и стартуем сервис
systemctl daemon-reload
systemctl enable nova-dashboard.service
systemctl restart nova-dashboard.service

echo "================================================================"
echo "🎉 NOVA Live WGDashboard успешно установлен и запущен!"
echo "🌐 Адрес в браузере: http://92.51.46.12:8088"
echo "👤 Логин: admin"
echo "🔑 Пароль: nova2026"
echo "================================================================"
